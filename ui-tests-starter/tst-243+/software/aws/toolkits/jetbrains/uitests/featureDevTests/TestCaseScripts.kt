...
async function testAcceptInitalCode() {
    ...
    try {
        ...
        await page.type('.mynah-chat-prompt-input', '/dev add current timestamp to TIMESTAMP.md in format YYYY-MM-DD hh:mm:ss')
...
async function testIterateCodeGen() {
    ...
    try {
        ...
        await page.type('.mynah-chat-prompt-input', 'Add current timestamp in TIMESTAMP.md ')
        await page.keyboard.press('Enter');
        ...
        // Second iteration
        await page.type('.mynah-chat-prompt-input', 'change the timestamp format to YYYY-MM-DD hh:mm:ss');
        await page.keyboard.press('Enter');
...
async function testPartialCodeGen() {
    ...
    try {
        ...
        await page.type('.mynah-chat-prompt-input', '/dev add current timestamp to TIMESTAMP.md')
        await page.keyboard.press('Enter');
        ...
        await retryIfRequired(page, async () => {
            ...
            // Accept code change
            await page.waitForSelector('.mynah-chat-item-followup-question-option', {timeout: 1000});
            const [acceptCode, iterateCode] = await page.$$('.mynah-chat-item-followup-question-option');
            await acceptCode.click();
